...
Your Server → Twilio → Physician/Pharmacist's Phone
     ...
     Contact speaks → Twilio records audio
                ↓
        Gemini transcription (Arabic transcription)
        ↓
      Gemini Conversation LLM (follows your script)
                ↓
...
│   │   ├── asr.js                 ← Gemini speech-to-text
│   │   ├── conversationLLM.js     ← Gemini script driver
...
|---|---|
| `MONGO_URI` | Use [MongoDB Atlas connection strings](https://www.mongodb.com/docs/atlas/driver-connection/) or keep `mongodb://localhost:27017/medcall` for local MongoDB |
| `JWT_SECRET` | Generate any long random string, for example with [Node.js crypto](https://nodejs.org/api/crypto.html#cryptorandombytessize-callback) or a password manager |
| `TWILIO_ACCOUNT_SID` | [Twilio Console dashboard](https://console.twilio.com/) |
| `TWILIO_AUTH_TOKEN` | [Twilio Console dashboard](https://console.twilio.com/) |
| `TWILIO_NUMBER` | Your purchased number in the [Twilio Phone Numbers console](https://console.twilio.com/us1/develop/phone-numbers/manage/incoming) |
| `BASE_URL` | Run `ngrok http 5000` and paste the public `https://xxxx.ngrok.io` URL |
| `GEMINI_API_KEY` | [Google AI Studio API keys](https://aistudio.google.com/app/apikey) |
| `AZURE_SPEECH_KEY` | [Azure Portal](https://portal.azure.com/) → your Speech resource → Keys and Endpoint |
| `AZURE_SPEECH_REGION` | The region shown on your Azure Speech resource, for example `eastus` |
...
3. Contact speaks
   └─ Twilio POSTs to /api/twilio/twiml/respond
      ...
      ├─ Gemini transcribes Arabic audio → text
      ├─ Gemini processes turn → next question + extractions
